...
const errorMessages = {
  ...
  // /remind-recurring
  recurringEndRequired: '❌ 請提供 `end_date` 或 `count` 其中一個（不能都不填，也不能都填）。',
  invalidRecurrenceEndDateFormat: '❌ 結束日期格式錯誤！請使用 `YYYYMMDD`，例如 `20261231`。',
  recurrenceEndDateBeforeStart: '❌ 結束日期不能早於第一次事件日期。',
  invalidRecurrenceCount: '❌ `count` 必須是大於 0 的整數。',
  invalidRecurrenceOffsetDays: '❌ `remind_offset_days` 必須是 0 或正整數。',
  ...
  // /remind-import：週期提醒欄位（type/remind_offset_days/end_date/count）
  csvLineInvalidRecurrenceType: (line, typeRaw) =>
    `第 ${line} 行：type 格式錯誤（\`${typeRaw}\`），請使用 daily/weekly/monthly`,
  csvLineRecurrenceEndRequired: (line) =>
    `第 ${line} 行：請提供 end_date 或 count 其中一個（不能都不填，也不能都填）`,
  csvLineInvalidRecurrenceEndDate: (line, endDateRaw) =>
    `第 ${line} 行：end_date 格式錯誤（\`${endDateRaw}\`），請使用 YYYYMMDD`,
  csvLineRecurrenceEndDateBeforeStart: (line) => `第 ${line} 行：end_date 不能早於事件日期`,
  csvLineInvalidRecurrenceCount: (line, countRaw) =>
    `第 ${line} 行：count 必須是大於 0 的整數（\`${countRaw}\`）`,
  csvLineInvalidRecurrenceOffsetDays: (line, offsetRaw) =>
    `第 ${line} 行：remind_offset_days 必須是 0 或正整數（\`${offsetRaw}\`）`,
