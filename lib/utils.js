...
const { defaultRemindHour, defaultRemindMinute, timezoneOffsetHours } = require('./config.json');
...
// 回傳該日期 UTC 00:00 的 Date 物件
function parseDateUTC(str) {
  return new Date(Date.UTC(+str.slice(0, 4), +str.slice(4, 6) - 1, +str.slice(6, 8)));
}

// UTC timestamp → 台灣時間（UTC+8）的 Date
function toTaipeiLocalDate(ts) {
  const taipei = addHours(new Date(ts), timezoneOffsetHours);
  return new Date(taipei.getTime() + taipei.getTimezoneOffset() * 60 * 1000);
}
...
function calcReminderTime(
  ...
  // setUTCHours 接受負值（remindHour < timezoneOffsetHours 時自動往前一天），行為正確
  remindDay.setUTCHours(remindHour - timezoneOffsetHours, remindMinute, 0, 0);
  return remindDay.getTime();
}

// ===== 週期提醒：日期推算（全部使用 UTC getter/setter，不受伺服器時區影響）=====

// /remind-recurring、/remind-import 共用的合法週期類型清單，新增類型時只需改這裡
const recurrenceTypes = ['daily', 'weekly', 'monthly'];

// Date（UTC）→ YYYYMMDD 字串，parseDateUTC 的反向操作
function formatDateStrUTC(dateUTC) {
...
// 加月份，若目標月無此日則 clamp 到當月最後一天（例：1/31 + 1 月 → 2/28 或 2/29）
...
// 依週期類型計算下一場次的事件日期（YYYYMMDD）
function calcNextOccurrenceDate(eventDateStr, type) {
  const current = parseDateUTC(eventDateStr);
  let next;
  switch (type) {
    case 'daily':
      ...
      break;
    case 'weekly':
      next = addDaysUTC(current, 7);
      break;
    case 'monthly':
      next = addMonthsUTCClamped(current, 1);
      break;
    default:
      throw new Error(`未知的週期類型 "${type}"`);
  }
  return formatDateStrUTC(next);
}
...
function formatTaipeiTime(ts) {
  return format(toTaipeiLocalDate(ts), 'yyyy/MM/dd HH:mm');
}

// 取得指定時刻（預設現在）的台灣時間日期字串（YYYYMMDD），供日期區間比較「今天」用
function getTaipeiDateStr(ts = Date.now()) {
  return format(toTaipeiLocalDate(ts), 'yyyyMMdd');
}
