...
一次最多顯示 25 筆，超出時會提示剩餘筆數，可使用 `/reminders-range` 縮小範圍查詢。

---

### `/reminders-range`

查看指定事件日期區間內的所有待發送提醒，一次最多顯示 25 筆。

| 參數 | 必填 | 說明 |
|------|------|------|
| from | 是 | 起始日期，格式 `YYYYMMDD`，例如 `20260601` |
| to | 否 | 結束日期，格式 `YYYYMMDD`，例如 `20260630`；不填則只查 `from` 當天 |
